...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql + pymysql://root:12345@localhost/model1"
...
def myth():
    ...
    def key(data_age, data_hyper, data_glu ,data_heart, data_bmi,  data_gen_male, data_gen_female, data_gen_other, data_marr_Y, data_marr_N, data_gvt, data_priv,data_self, data_Nev,data_Child, data_rural,data_urban, data_unk_sm, data_use_sm, data_nev_sm , data_sta_sm ):
        ...
        if  data_gen_male == True:
            ...
            key_gender = 2 
        sum_g = 0   
        for gender in (data_gen_male, data_gen_female, data_gen_other):
            if gender == True:
                sum_g += 1
            if sum_g >= 2:
                flash('Nhập quá 2 giới tính.')
                break
        ...
        if data_marr_N == True:
            ...
            key_marr = 1 
        key_m = 0    
        for marrr in (data_marr_N, data_marr_Y):
            if marrr == True:
                key_m += 1
            if key_m == 2:
                flash('Nhập sai tình trạng hôn nhân.')
                break
        ...
        if data_gvt == True:
            ...
            key_job = int(4)
        k_job = 0 
        for job in (data_Child,data_self, data_gvt, data_Nev,data_priv):
            if job == True:
                 k_job += 1
            if  k_job >= 2:
                flash('Nhập dư nghề nghiệp.')
                break
        ...
        key_l = 0    
        for live in (data_rural, data_urban):
            if live == True:
                key_l += 1
            if key_l == 2:
                flash('Nhập thừa nơi sống.')
                break    
    # Tiểu sử hút thuốc
        if data_unk_sm == True:
            key_smoke = int(0)  
            ...
            key_smoke = int(3) 
        key_mk = 0    
        for marrr in (data_unk_sm, data_use_sm, data_nev_sm, data_sta_sm):
            if marrr == True:
                key_mk += 1
            if key_mk >= 2:
                flash('Nhập dư tiền sử hút thuốc.')
                break
        # trả về tham số    
